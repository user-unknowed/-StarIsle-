...
RE_NAME_EN_2 = re.compile(r"\b([A-Z][a-z]{1,15})\s+([A-Z][a-z]{1,15})\b")
RE_NAME_EN_3 = re.compile(r"\b([A-Z][a-z]{1,15})\s+([A-Z])\.?\s+([A-Z][a-z]{1,15})\b")
...
def _mask_value(v: Any, st: MaskStats) -> Any:
    if isinstance(v, str): return _mask(v, st)
    if isinstance(v, list): return [_mask_value(x, st) for x in v]
    if isinstance(v, dict): return {k: _mask_value(x, st) for k, x in v.items()}
    return v
